...
class RecipeCreation(CBPiExtension):
    ...
    async def kbh_recipe_creation(self, Recipe_ID):
        ...
        if self.kettle is not None:
            ...
            try:
                ...
                self.cbpi.notify('KBH Recipe creation failure: {}'.format(e), name, NotificationType.ERROR)
                pass
        else:
    ...
    async def json_recipe_creation(self, Recipe_ID):
        ...
        try:
            ...
            self.cbpi.notify('MMuM-JSON Recipe creation failure: {}'.format(e), name, NotificationType.ERROR)
            logger.error(e)

    async def xml_recipe_creation(self, Recipe_ID):
        config = self.get_config_values()
        logger.error("XML Recipe Creation")
        logger.error(config)    
        try:
            if self.kettle is not None:
            ...
            self.cbpi.notify('BeerXML Recipe creation failure: {}'.format(e), name, NotificationType.ERROR)
            logger.error(e)
            pass
